...
import MOD_ACTIONS, { isAiFullTtsEnabled } from './actions/modActions';
...
const messageHandler = (chat: tmi.Client): OnNewMessage => async ({ channel, message, tags, ttsUser, self }) => {
  ...
  if (!formattedMessage.startsWith('!') && isAiMention(formattedMessage)) {
    ...
    const result = await askAiQueued(channel, username, formattedMessage);
    ...
    if (result?.command) {
      ...
      const commandMessage = commandValue
