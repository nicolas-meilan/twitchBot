...
export type AiExternalAction = typeof AI_EXTERNAL_ACTIONS[keyof typeof AI_EXTERNAL_ACTIONS];
...
const parseCommand = (value: unknown): AiCommand | undefined | null => {
  ...
  return {
    name: normalizedName.startsWith('!') ? normalizedName : `!${normalizedName}`,
...
const parseExternalInformation = (value: unknown): AiExternalInformationRequest | undefined | null => {
  if (value === undefined || value === null) return undefined;
  if (!isRecord(value) || !Object.values(AI_EXTERNAL_ACTIONS).includes(value.action as AiExternalAction)) return null;
