...
export const COMMAND_DESCRIPTIONS: Record<string, CommandDescription> = {    
  ...
  [LAST_RANKED_KEY]: withArguments(`mostrar el resultado de la última partida ranked. Acepta un usuario#tag de LATAM; si no se indica, consulta al streamer (por defecto: ${DEFAULT_VALORANT_USERNAME}#${DEFAULT_VALORANT_TAG})`, `${LAST_RANKED_KEY} [usuario#tag]`),    
  [LAST_RANKED_ALIAS_KEY]: withArguments(`mostrar el resultado de la última partida ranked. Acepta un usuario#tag de LATAM; si no se indica, consulta al streamer (por defecto: ${DEFAULT_VALORANT_USERNAME}#${DEFAULT_VALORANT_TAG})`, `${LAST_RANKED_ALIAS_KEY} [usuario#tag]`),  
  [LAST_RANKED_2_KEY]: withArguments(`mostrar el resultado de la última partida ranked. Acepta un usuario#tag de LATAM; si no se indica, consulta al streamer (por defecto: ${DEFAULT_VALORANT_USERNAME}#${DEFAULT_VALORANT_TAG})`, `${LAST_RANKED_2_KEY} [usuario#tag]`),  
  [LAST_GAME_KEY]: withArguments(`mostrar el resultado de la última partida ranked. Acepta un usuario#tag de LATAM; si no se indica, consulta al streamer (por defecto: ${DEFAULT_VALORANT_USERNAME}#${DEFAULT_VALORANT_TAG})`, `${LAST_GAME_KEY} [usuario#tag]`),  
  ...
  [CHANGE_CHANNEL_INFORMATION_KEY]: withArguments('cambiar la información del stream mediante categoría, título y tags. La categoría es obligatoria; el título y los tags son opcionales. La categoría se indica por su nombre y el sistema obtiene automáticamente su ID. El título de Twitch admite hasta 140 caracteres y puede contener espacios, acentos, emojis y caracteres especiales. Los tags admiten un máximo de 10 tags, con un máximo de 25 caracteres por tag. Los tags NO deben incluir #, espacios ni caracteres especiales. Para evitar errores, los tags deben contener únicamente letras y números, sin acentos ni símbolos. Los tags deben estar separados por comas. Los tags enviados reemplazan los tags anteriores del canal. No uses hashtags.', `${CHANGE_CHANNEL_INFORMATION_KEY} CATEGORIA [/ TITULO] [/ TAG1,TAG2,TAGX]`),    
  [CHANGE_CHANNEL_INFORMATION_KEY_2]: withArguments('cambiar la información del stream mediante categoría, título y tags. La categoría es obligatoria; el título y los tags son opcionales. La categoría se indica por su nombre y el sistema obtiene automáticamente su ID. El título de Twitch admite hasta 140 caracteres y puede contener espacios, acentos, emojis y caracteres especiales. Los tags admiten un máximo de 10 tags, con un máximo de 25 caracteres por tag. Los tags NO deben incluir #, espacios ni caracteres especiales. Para evitar errores, los tags deben contener únicamente letras y números, sin acentos ni símbolos. Los tags deben estar separados por comas. Los tags enviados reemplazan los tags anteriores del canal. No uses hashtags.', `${CHANGE_CHANNEL_INFORMATION_KEY_2} CATEGORIA [/ TITULO] [/ TAG1,TAG2,TAGX]`),    
