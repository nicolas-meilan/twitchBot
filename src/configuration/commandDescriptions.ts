...
export const COMMAND_DESCRIPTIONS: Record<string, CommandDescription> = {
  ...
  [VALORANT_RANK_ALIAS_KEY]: withArguments('mostrar el rango actual de Valorant para un usuario de LATAM o para el streamer si no se pasa valor', `${VALORANT_RANK_KEY} [usuario#tag]`),
  [VALORANT_ELO_KEY]: withArguments('mostrar el rango actual de Valorant para un usuario de LATAM o para el streamer si no se pasa valor', `${VALORANT_RANK_KEY} [usuario#tag]`),
  [VALORANT_RANK_ALIAS_2_KEY]: withArguments('mostrar el rango actual de Valorant para un usuario de LATAM o para el streamer si no se pasa valor', `${VALORANT_RANK_KEY} [usuario#tag]`),
  [VALORANT_KEY]: withArguments('mostrar el rango actual de Valorant para un usuario de LATAM o para el streamer si no se pasa valor', `${VALORANT_RANK_KEY} [usuario#tag]`),
  [VALORANT_ID_ALIAS_KEY]: withArguments('mostrar el rango actual de Valorant para un usuario de LATAM o para el streamer si no se pasa valor', `${VALORANT_RANK_KEY} [usuario#tag]`),
  ...
  [LAST_RANKED_ALIAS_KEY]: withArguments('mostrar el resultado de la última partida ranked de un usuario de LATAM o del streamer si no se pasa valor', `${LAST_RANKED_KEY} [usuario#tag]`),
  [LAST_GAME_KEY]: withArguments('mostrar el resultado de la última partida ranked de un usuario de LATAM o del streamer si no se pasa valor', `${LAST_RANKED_KEY} [usuario#tag]`),
  ...
  [FULL_TTS_KEY]: noArguments('activar o desactivar el modo TTS completo de la IA; distinto de !tts porque hace que todas las respuestas de la IA salgan por voz sin límite de caracteres'),
